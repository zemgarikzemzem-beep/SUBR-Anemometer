...
void NOKIA_SetPos(uint8_t x, uint8_t y){
	N_COMMAND;
	SPI1_Send_Byte(x|0x80);
	delay(10);                   // Таймаут обязателен!!!
	SPI1_Send_Byte(y|0x40);
	delay(10);
}
...
void NOKIA_Clean(void){
	NOKIA_SetPos(0,0);
	N_DATA;
	for (uint16_t i=0; i<504; ++i) SPI1_Send_Byte(0x00);
}

void NOKIA_Init(void){
	GPIOB->MODER&=~0x000F0000;
	GPIOB->MODER|=((GPIO_MODE_OUTPUT<<(8<<1))|(GPIO_MODE_OUTPUT<<(9<<1)));
	GPIOB->OTYPER|=((GPIO_OUTPUT_PUSHPULL<<8)|(GPIO_OUTPUT_PUSHPULL<<9));
	GPIOB->OSPEEDR|=((GPIO_SPEED_FREQ_HIGH <<(8<<1))|(GPIO_SPEED_FREQ_HIGH <<(9<<1)));
...
void NOKIA_Send_Char(uint8_t ch){
	N_DATA;
	if(ch>=192) for (uint8_t i=0; i<5; ++i) SPI1_Send_Byte(chars[ch-96][i]);
	else for (uint8_t i=0; i<5; ++i) SPI1_Send_Byte(chars[ch-32][i]);
	SPI1_Send_Byte(0x00);
}

void NOKIA_Send_Str(char* str){
	uint8_t i=0;
	while(str[i]!=0) {NOKIA_Send_Char(str[i]);++i;}
}
static const char lcd_image_mas[504] =
{ 0x1C, 0x22, 0x22, 0x22, 0x00, 0x00, 0x00, 0x3E, 0x8A, 0x8A, 0x04, 0x00,
  ...
  };


void NOKIA_Monkey(void){
	N_DATA;
	for(uint16_t i=0;i<504;++i) { SPI1_Send_Byte(lcd_image_mas[i]);}
}
